...
void LoginServerClient::connectionChangeCallback(const mg::TcpConnectionPointer &link, mg::EventLoop *loop)
{
    if (link->connected())
    {
        ...
        link->runEvery(SERVER_HEARTBEAT_INTERVAL / 1000000, std::bind(&LoginServerClient::heartBeatMessage, this, link));
...
void LoginServerClient::disableRetry()
{
    this->_client->disableRetry();
}
