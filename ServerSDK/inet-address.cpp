...
mg::InternetAddress::InternetAddress(uint16_t port, bool isIpv6)
{
    if (isIpv6)
    {
        ...
        _address6.sin6_addr = in6addr_any;
    ...
    else
    {
        ...
        _address4.sin_addr.s_addr = ::htonl(INADDR_ANY);
    }
}

mg::InternetAddress::InternetAddress(const sockaddr_in &address) : _address4(address) {}

mg::InternetAddress::InternetAddress(const sockaddr_in6 &address) : _address6(address) {}
...
std::string mg::InternetAddress::toIpPort() const
{
    ...
    if (_ipv6)
    {
        ::inet_ntop(AF_INET6, &_address6.sin6_addr, buf, sizeof(buf));
        int len = ::strlen(buf);
        uint16_t port = ::ntohs(_address6.sin6_port);
        snprintf(buf + len, sizeof(buf) - 1, ":%u", port);
    }
    else
    {
        ::inet_ntop(AF_INET, &_address4.sin_addr, buf, sizeof(buf));
        int len = ::strlen(buf);
        uint16_t port = ::ntohs(_address4.sin_port);
        snprintf(buf + len, sizeof(buf) - 1, ":%u", port);
    }
    return buf;
}
...
{
    return ::ntohs(_ipv6 ? _address6.sin6_port : _address4.sin_port);
}
